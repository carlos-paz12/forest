 ...
 * @return value_type  The balance factor of the node, which is the difference between the heights of the left and right
 * subtrees.
 */
...
TreeNodePtr r_simple_left(TreeNodePtr& node)
{
  ...
  temp->height =
    std::max(height_no(temp->m_left), height_no(temp->m_right)) + 1; //!< Updates the height of the temporary node.
