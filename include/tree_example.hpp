#ifndef TREE_EXAMPLE_HPP
#define TREE_EXAMPLE_HPP
/**
 * @file tree.hpp
 * @brief  Implementação de uma árvore binária de busca com nós contendo pares ordenados.
 * @details Esta árvore permite a inserção, remoção e busca de nós baseados em chaves geradas a partir de pares
 * ordenados. A chave é gerada usando a fórmula de Cantor, garantindo que cada par ordenado tenha uma chave única. A
 * árvore é impressa de forma legível, mostrando a estrutura hierárquica dos nós.
 * @version 0.1
 * @date 2025-06-16
 *
...
struct Node
{
  ...
  : key(key), m_data(data), m_left(nullptr), m_right(nullptr)
  { /* empty */
  }
};
...
TreeNodePtr insert(TreeNodePtr& tree, const value_type key, const User& data)
{
  ...
  if (key < tree->key)
  {
    insert(tree->m_left, key, data);
  } //<! Se a chave for menor que a chave do nó atual, insere à esquerda.

  else if (key > tree->key)
  {
    insert(tree->m_right, key, data); // se a chave for maior, insere à direita.
  }
 ...
 */
void print_tree(const TreeNodePtr& tree, const std::string& prefix = "", bool is_left = false, bool is_root = true)
{
  std::cout << prefix;
  std::cout << (is_root ? ".\n└──" : (is_left ? "├──" : "└──"));
  ...
  if (tree != nullptr)
  {
    std::cout << "\033[32m[ key:" << tree->key << "] { \"name:\" \"" << tree->m_data.login << "\", \"userid:\" "
              << tree->m_data.userid << ", \"region:\"" << tree->m_data.region << ", \"passoword:\" \""
              << tree->m_data.password << "}\033[0m\n";
